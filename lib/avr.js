...
const Direct = require('../accessories/Direct');
const migrateLegacyPersist = require('./persistMigration')
const { syncInputButtons } = require('./inputButtons')
	...
	init: async function() {
		...
		for (const config of this.receivers) {
			...
			if (deviceConfig) {
				...
				deviceConfig.partySwitch = !!config.partySwitch
				deviceConfig.directSwitch = !!config.directSwitch
			...
			inputButtonReceivers.push({ avr, config, device: deviceConfig })
		}

		syncInputButtons(this, inputButtonReceivers)
...
const createNewConfig = async (config, availableInputs, log) => {
	...
	try {
		const newConfig = {
			...
			id: config.id,
			model: config.model,
			partySwitch: config.partySwitch,
			directSwitch: config.directSwitch,
			zone1: {
				name: config.name,
...
const newAVR = function(avr, deviceConfig, platform) {
	...
	if (deviceConfig.partySwitch) {
		platform.log.easyDebug(`Party Mode Switch enabled for ${deviceConfig.zone1.name}`);
		new Party(avr, platform, getZoneConfig(deviceConfig, 1))
	}

	if (deviceConfig.directSwitch) {
		platform.log.easyDebug(`Direct Switch enabled for ${deviceConfig.zone1.name}`);
		new Direct(avr, platform, getZoneConfig(deviceConfig, 1))
	}
